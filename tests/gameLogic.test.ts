...
describe('getGuessResult', () => {
  ...
  describe(`${correctWord} as correct word`, () => {
    it('ZAMEK', () => {
      const result = getGuessResult('ZAMEK', correctWord).letters.map((letter) => ({
    ...
    it('LAMPA', () => {
      const result = getGuessResult('LAMPA', correctWord).letters.map((letter) => ({
        exists: letter.exists,
        correctPlace: letter.correctPlace,
      }));
    ...
    it('LAMAA', () => {
      const result = getGuessResult('LAMAA', correctWord).letters.map((letter) => ({
        exists: letter.exists,
        correctPlace: letter.correctPlace,
      }));
    ...
    it("LLLLL — duplicate letters, only 2 L's should be highlighted", () => {
      const result = getGuessResult('LLLLL', correctWord).letters.map((letter) => ({
        exists: letter.exists,
        correctPlace: letter.correctPlace,
      }));
    ...
    describe('edge cases', () => {
      it('correct guess — all greens', () => {
        const result = getGuessResult('LALKA', correctWord).letters.map((letter) => ({
          exists: letter.exists,
          correctPlace: letter.correctPlace,
        }));
      ...
      it('no letters match — all grays', () => {
        const result = getGuessResult('ZEROS', correctWord).letters.map((letter) => ({
          exists: letter.exists,
          correctPlace: letter.correctPlace,
        }));
      ...
      it('duplicate letter in guess, only one in correct word — only one yellow', () => {
        ...
        const result = getGuessResult('KKNNN', correctWord).letters.map((letter) => ({
          exists: letter.exists,
          correctPlace: letter.correctPlace,
        }));
      ...
      it('green takes priority over yellow for duplicate letters', () => {
        ...
        const result = getGuessResult('AAXXX', correctWord).letters.map((letter) => ({
          exists: letter.exists,
          correctPlace: letter.correctPlace,
        }));
  ...
  describe('input validation', () => {
    it('throws error for empty string', () => {
      expect(() => getGuessResult('', correctWord)).throw('Guess cannot be empty');
    });
